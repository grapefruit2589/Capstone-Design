...
class SokdamQuiz extends StatelessWidget {
  @override
...
class _SokdamQuizGameState extends State<SokdamQuizGame> {
  ...
  void _startTimer() {
    _timer?.cancel();
    _timer = Timer(Duration(seconds: 10), _nextQuestion);
  }
  ...
  void _nextQuestion() {
    ...
    if (_currentQuestion < 5) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(
          builder: (context) => SokdamResultScreen(score: _score),
        ),
      );
    }
  }
...
class SokdamResultScreen extends StatelessWidget {
  final int score;

  SokdamResultScreen({required this.score});
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              ...
              child: Text('게임 선택 화면으로', style: TextStyle(fontSize: 18)),
            )
          ],
        ),
      ),
